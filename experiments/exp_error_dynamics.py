...
if __name__ == "__main__":
    ...
    amounts_extra_l = np.linspace(0, 15, 35)
    ...
    for i, beta in enumerate(beta_values):
        plt.plot(amounts_extra_l, [avg_pct[i] for avg_pct in all_avg_pct_norm_abandonment], label=f'Beta = {beta}')

    plt.xlabel('Addition to minimum incentive needed for changes')
    plt.ylabel('Percentage of Norm Abandonment')
    plt.title('Effect of Beta on Percentage of Norm Abandonment')
    plt.legend()
    plt.grid(True)
    plt.savefig("trans_2pct_norm_abandonment_vs_total_to_distribute.png")
    plt.close()