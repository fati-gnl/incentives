...
def run_experiment_for_strategy(incentive_strategy, G, total_to_distribute_values, size_network, random_seed, Vl, p,
                                model_steps, beta):
    """
    ...
    for total_to_distribute in total_to_distribute_values:
        ...
        print("Received incentive but did not transition count: " + str(
            model.inc_but_no_transition)) if model.inc_but_no_transition != 0 else None
    ...
    return (
    incentive_strategy, results_timesteps_95, results_spillovers, results_incentivized, results_norm, results_gini)
...
if __name__ == '__main__':
    ...
    if network_type == "Erdos_Renyi":
        total_to_distribute_values = np.linspace(35000, 70000, 3)
    ...
    incentive_strategies = ["High_local_clustering_c"] # Betweenness_centrality "Random", "Highest_degree", "Lowest_degree", "Highest_gamma", "Lowest_gamma"
    ...
    def calculate_average_values(data, incentive_strategies):
        """
        Calculate the average values for each incentive level across multiple runs.
        """
        strategy_names = incentive_strategies
    ...
    for metric, results in results_all_sim.items():
        average_results[metric] = calculate_average_values(results, incentive_strategies)
    ...
    # METRICS:
    # Total cost used vs total number of people who have transitioned
    # Ratio of the total incentives distributed to the total number of transitions achieved.
    # Gini coefficient
    ...
    for i, (metric, results) in enumerate(average_results.items()):
        plot_and_save_results(results, 'res_{}_{}_{}.png'.format(network_type, entitled_distribution, file_titles[i]), total_to_distribute_values, ylabels[i], incentive_strategies)
