"""
local_sensitivity_analysis.py

In this file, a global sensitivy analysis has been conducted to assess the minimum
incentive amount required to trigger the complete abandonment of a detrimental norm
within a heterogeneous networked population (out of the instances that reach complete norm abandonment).
"""
...
# Sample points used for sensitivity analysis
...
def run_experiment_for_strategy(num_runs, param_value, incentive_strategy,
                                ...
                                random_seeds):
    """
    This function determines for a given parameter set the minimum incentive amount required
    to trigger the complete abandonment of a detrimental norm.
    ...
    :return: num_runs, param_value, incentive_strategy, scaled_value
    """
    ...
    # Binary search for smallest total_to_distribute_value that meets the condition
    low = 0
    high = len(total_to_distribute_values) - 1
    smallest_total_to_distribute = None
    scaled_value = None

    while low <= high:
        mid = (low + high) // 2
        total_to_distribute_value = total_to_distribute_values[mid]
        ...
        if game_model.pct_norm_abandonmnet[-1] == 1:
            smallest_total_to_distribute = total_to_distribute_value
            high = mid - 1  # Search for a potentially smaller value
        else:
            low = mid + 1  # Search for a larger value

    # Scale the total to distribute values accordinly.
    if smallest_total_to_distribute is None:
        print("Failed to find smallest_total_to_distribute")
        print("failed values:", param_value)
    else:
        scaled_value = (smallest_total_to_distribute - min(total_to_distribute_values)) / (
                max(total_to_distribute_values) - min(total_to_distribute_values))
...
if __name__ == '__main__':
    ...
    for result in results:
        num_runs, param_value, incentive_strategy, scaled_value = result
        ...
        Y[ind_incentive_strategy][ind_param_value].append(scaled_value)

    print("Y", np.array(Y))

    # Aggregates the results per incentive strategy for the different simulation runs by using the mean
    Y_nan = [[[np.nan if val is None else val for val in sublist] for sublist in sublist_list] for sublist_list in Y]
    Y1 = [[np.nanmean(sublist) if np.any(~np.isnan(sublist)) else 2 for sublist in sublist_list] for sublist_list in
          Y_nan]
    ...
    results = sb_analyze.analyze(problem, Y_final, calc_second_order=True, print_to_console=False)
    ...
    results.plot()
    plt.title(f"Sobol Indices")
    plt.tight_layout()
    plt.savefig("sobol_indices99.png")
    plt.close()